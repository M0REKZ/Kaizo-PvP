...
CLaserPvP::CLaserPvP(CGameWorld *pGameWorld, vec2 Pos, vec2 Direction, float StartEnergy, int Owner, int Type) :
	CEntity(pGameWorld, CGameWorld::KZ_ENTTYPE_LASER_PVP)
{
	m_Pos = Pos;
	m_Owner = Owner;
...
bool CLaserPvP::HitCharacter(vec2 From, vec2 To)
{
	...
	pHit->TakeDamage(vec2(0, 0), 5, m_Owner, m_Type);
	return true;
}

void CLaserPvP::DoBounce()
{
...
void CLaserPvP::TickPaused()
{
	++m_EvalTick;
}

void CLaserPvP::Snap(int SnappingClient)
{
...
void CLaserPvP::SwapClients(int Client1, int Client2)
{
